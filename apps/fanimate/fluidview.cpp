...
{
  (void)timeStep; //Avoid unused warning
  glutInit(argc, argv);
...
#define USE_track_velocity
  ...
  vMax.x = std::max(vMax.x * fptype(0.99), vMin.x + epsilon);
  vMax.y = std::max(vMax.y * fptype(0.99), vMin.y + epsilon);
  vMax.z = std::max(vMax.z * fptype(0.99), vMin.z + epsilon);
#else
  prior_y_Max = vMax.y;
...
void Keyboard(unsigned char key, int, int) // Avoid unused warning
{
  switch(key)
  {
