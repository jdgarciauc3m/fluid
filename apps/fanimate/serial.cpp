...
// 14-Feb-2013 Modified by J. Daniel Garcia
// + Changed ifs based on unspecified behaviour (address comparisons) causing bugs
// 15-Feb-2013 Modified by J. Daniel Garcia
// + Removed parsec hooks and added timing utility (xul::time_meter)
...
float restParticlesPerMeter; // Bug: Must be float and not fptype
fptype h, hSq;
        ...
        for(int ipar = 0; ipar < np; ++ipar)
        {
          for(int inc = 0; inc < numNeighCells; ++inc)
          {
            ...
            for(int iparNeigh = 0; iparNeigh < numNeighPars; ++iparNeigh)
            {
              ...
              if ( (cindex > cindexNeigh) || (cindex == cindexNeigh && ipar > iparNeigh))
              {
...
{
	//int x,y,z, index;
	int x,y,z; // Removed unused index
...
//#define USE_ImpeneratableWall <-- Use -D to set
